...
RUN cp  /etc/apt/sources.list /etc/apt/sources.list.bak && sed -i -re 's/([a-z]{2}\.)?archive.ubuntu.com|security.ubuntu.com/old-releases.ubuntu.com/g' /etc/apt/sources.list &&apt-get update && apt-get install -y --no-install-recommends \
    python3 iputils-ping tzdata \ 
    && apt-get install dos2unix \ 
    && apt-get autoremove -y \
    && dos2unix /home/steam/edit_server_config.py \
    && sed -i 's/\r$//' /home/steam/run_server.sh && rm -rf /var/lib/apt/lists/*
